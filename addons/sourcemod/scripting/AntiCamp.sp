...
#define GREEN				"\x04"
#define PLUGIN_VERSION 		"2.5.2fix"
...
{
	CreateConVar("sm_anticamp_version", PLUGIN_VERSION, "sm_anticamp_version", FCVAR_NOTIFY|FCVAR_DONTRECORD);
	...
	g_CvarBlind = CreateConVar("sm_anticamp_blind", "1", "Blind a player while camping", 0, true, 0.0, true, 1.0);
	...
	g_CvarMinHealth = CreateConVar("sm_anticamp_minhealth", "0", "Minimum health a camper reserves. Set 0 to slap till dead. If slay set the health from which player will not be killed", 0, true, 0.0, true, 100.0);
	g_CvarPunishDelay = CreateConVar("sm_anticamp_punish_delay", "3", "Delay between camper notification and first punishment in secounds", 0, true, 0.0, true, 60.0);
	g_CvarPunishFreq = CreateConVar("sm_anticamp_punish_freq", "3", "Time between punishments while camping in secounds", 0, true, 1.0, true, 60.0);
	g_CvarPunishAnyway = CreateConVar("sm_anticamp_minhealth_camp", "0", "Set 0 to allow camping below minhealth. Set 1 to punish without damage", 0, true, 0.0, true, 1.0);
	g_CvarRadius = CreateConVar("sm_anticamp_radius", "120", "The radius to check for camping", 0, true, 50.0, true, 500.0);
	g_CvarCampTime = CreateConVar("sm_anticamp_camptime", "15", "The amount of times a suspected camper is checked for", 0, true, 2.0, true, 60.0);
...
public Action:CaughtCampingTimer(Handle:timer, any:client)
{
	...
	else
	{
		...
		for(new i=1; i<=MaxClients; i++)
		{
			...
			{
				Format(Saytext, sizeof(Saytext), " \x01[ \x02ZK Servidores™ \x01] \x01%T", "Player camping", i, name,weapon,place,YELLOW,TEAMCOLOR,YELLOW,GREEN,YELLOW,GREEN);
...
public Action:PunishTimer(Handle:timer, any:client)
{
	...
	if(GetConVarBool(g_CvarBeacon))
	{
		...
		EmitSoundToClient("buttons/button17.wav", client, SNDCHAN_AUTO, SNDLEVEL_NORMAL, SND_NOFLAGS, 1.0, SNDPITCH_NORMAL, -1, vecPos, NULL_VECTOR, true, 0.0);
	}
